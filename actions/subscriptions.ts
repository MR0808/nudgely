...
> {
  ...
  try {
    ...
    const plan = await prisma.plan.findFirst({
      where: {
        OR: [{ stripeMonthlyId: pending.priceId }, { stripeYearlyId: pending.priceId }],
      },
    });
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        invoices,
        nextBillingDate: subscription.items.data[0].current_period_end ?? null,
      },
    };
  } catch (error) {
    return {
...
export async function checkDowngradedPlan(companyId: string): Promise<ActionResult<true>> {
  try {
    const company = await prisma.company.findUnique({
      where: { id: companyId },
      include: {
    ...
    // Count nudges
    const totalNudges = company.teams.reduce((acc, team) => acc + team.nudges.length, 0);
